...
/* 
 * Instead of unconditionally defining these macros, we guard them.
 */
#ifndef MD5_CBLOCK
...
/*
 * cookie_expires() returns a string (in the buffer provided) with the cookie expiration date.
 */
static ngx_int_t
cookie_expires(char *str, size_t size, time_t t)
{
...
/*
 * ngx_http_sticky_misc_set_cookie()
 *
 * Sets the Set-Cookie header in the response.
 */
ngx_int_t
ngx_http_sticky_misc_set_cookie(ngx_http_request_t *r, ngx_str_t *name, ngx_str_t *value,
                                ngx_str_t *domain, ngx_str_t *path, time_t expires,
                                unsigned secure, unsigned httponly)
{
    u_char  *cookie, *p;
    size_t   len;
 ...
 */
ngx_int_t
ngx_http_sticky_misc_md5(ngx_pool_t *pool, void *in, size_t len, ngx_str_t *digest)
{
    ngx_md5_t md5;
 ...
 */
ngx_int_t
ngx_http_sticky_misc_sha1(ngx_pool_t *pool, void *in, size_t len, ngx_str_t *digest)
{
    ngx_sha1_t sha1;
